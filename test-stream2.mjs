...
async function test() {
  ...
  // Consume the fullStream to see all events
  ...
  for await (const part of result.fullStream) {
    if (part.type === "text-delta") {
      ...
      console.log(`[tool-result] ${part.toolName}: ${JSON.stringify(part.result).substring(0, 200)}`);
    } else if (part.type === "step-start") {
      console.log(`[step-start] step`);
    } else if (part.type === "step-finish") {
      console.log(`[step-finish] finishReason: ${part.finishReason}`);
